...
- Footer: "Hasura Bot: last read 2m ago · 3 pending · next nudge in 1m · Sync now", plus "· 2 nudged, not yet read" while `unread > pending`. "Last read" is the bot's last pull, not the last doorbell.
- Below the footer, a monospace debug block (`data-testid="debug-state"`) prints the raw state every poll returns: log position (`seq`, events loaded, posting count, poll interval), both bot cursors (`nudged`, `pulled`) with `pending`/`unread`, the nudge window (`due`, `inflight`, oldest pending, due at, window size/count), last nudge (with `message_id`) and last pull, presence names and build id. The banner carries no usage instructions — those live in the toolbar.
- Every poll response also carries `presence {count, viewers, ttlMs}` (who polled within `PRESENCE_TTL_MS`, per person; default 6 s = 1.5 tab polls, so a closed tab disappears for others within one poll after the TTL, ≤10 s) and `build` (hash of the served `index.html`). The commenting bar shows a 👥 count on the left with the names in its tooltip; when `build` changes from the one the tab loaded, a **red ⟳ refresh** control appears in the bar (tooltip: *the app was updated, refresh to see the changes, your comments are saved*). Presence is in-memory; a restart forgets it and the next poll rebuilds it.
