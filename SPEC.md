...
- Banner (v0.3.5): title, then one status line — "Signed in: Alice · Hasura Bot last read 2m ago" and, only while something is pending, "· 3 pending [· nudge due] Sync now"; "Reconnecting…" after two missed polls. When the served build differs from the one the tab loaded, a one-line "App updated — Refresh to see the changes. Your comments are saved." appears in the banner as well as the red ⟳ in the bar. "Last read" is the bot's last pull, not the last doorbell.
