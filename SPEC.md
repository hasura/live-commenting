...
- The poll response carries `sync.due: true` when the **oldest** *user* event past `nudged` is ≥ `SYNC_MAX_AGE` old (default **2 min**; must stay < the 15-min VM idle window) or such events number ≥ `SYNC_MAX_COUNT` (default 50). Whichever open tab sees `due` calls `POST /api/sync-now`; that request carries a fresh proxy-injected visitor token, which is the actor. The server dedupes to one in-flight send; concurrent callers get `204`. A reviewer can also click **Sync now**.
...
- Why two cursors: if the only cursor moved on pull, `due` would stay true and every 4 s poll would ring again until the bot pulled. Measuring `due` against `nudged` gives exactly one doorbell per new batch (2 min after its oldest comment), even while the bot is busy; a pull via read-before-work before the doorbell is due moves `nudged` too, so no stale doorbell follows. No cooldown or other dedupe is needed.
...
- Banner (v0.3.6): title, then one status line — "Signed in: Alice · Bot last read 2m ago" and, only while something is pending, "· 3 pending [· nudge due] Sync now"; "Reconnecting…" after two missed polls. The app-updated signal lives only in the bar (red ⟳); the banner carries no separate line for it. "Last read" is the bot's last pull, not the last doorbell. Bot actors are shown by name alone (`BOT_NAME`, default `Bot`) — no "(bot)" suffix anywhere.
- Everything else lives in a collapsed `<details>` titled **Live commenting debug** (`data-testid="debug-state"`): log position (`seq`, events loaded, posting count, poll interval, OFFLINE), both bot cursors (`nudged`, `pulled`) with `pending`/`unread` and "N nudged, not yet read", the nudge window (`due`, `inflight`, oldest pending, due at, next nudge, window size/count), last nudge (with `message_id`) and last pull, presence names with `ttl (poll + grace)`, build id. The banner carries no usage instructions — those live in the toolbar.
- Every poll response also carries `presence {count, viewers, ttlMs, pollMs, graceMs}` and `build` — the `BUILD_ID` env var, or the server's start time when unset (a content hash was dropped: it is unreliable when a rebuild changes only referenced assets, and the deployer knows when it deployed). Presence TTL = `POLL_MS + PRESENCE_GRACE_MS` (4 s + 2 s): a viewer expires 6 s after their last poll and other tabs see it on their own next poll, 6–10 s after. A TTL below the poll interval would drop every viewer between their own polls, so it is expressed as poll + grace rather than an absolute number. Tabs take `pollMs` from the server. The commenting bar shows a 👥 count on the left with the names in its tooltip; when `build` changes from the one the tab loaded, a **red ⟳ refresh** control appears in the bar (tooltip: *the app was updated, refresh to see the changes, your comments are saved*). Presence is in-memory; a restart forgets it and the next poll rebuilds it.
